...
# this file is used to download data from twitter and save in subdirectories of results
...
def main():
    ...
    begin = True
    folder_name = var.start[0:10] + "_" + \
        var.end[0:10]  # year-mo-da_year-mo-da
    ...
    page = 0
    while True:
        # folder_name/0.txt, folder_name/1.txt, folder_name/2.txt ...
        ...
        json_response = connect_to_endpoint(
            search_url, headers, query_params)  # search in twitter API
        print(json.dumps(json_response, indent=4, sort_keys=True))
        with open(path + filename, 'w') as file:
            json.dump(json_response, file, indent=4,
                      sort_keys=True)  # save file
            file.close()
        # if is the last page, break
