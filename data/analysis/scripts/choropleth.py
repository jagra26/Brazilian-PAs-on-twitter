        ...
        for index in df.index:
            try:
                ...
                if countries.get(alpha_3) == None:
                    ...
                    countries.update(
                        {alpha_3: int(quant + countries.get(alpha_3))})
                    countriesID.update(
                        {alpha_3: countriesID.get(alpha_3)+"/"+str(df.ID[index])})
                if alpha_3 == "BRA":
                    if state == None:
                        ...
                        statesID.update(
                            {state: statesID.get(state)+"/"+str(df.ID[index])})
                time.sleep(0.05)
                bar()
...
# This section is used to generate databases for countries and states
# The input database (resources/coordenadas sumarizadas_lat e long.xlsx) is on form:
# ID	latitude	longitude	quantidade
# The iso file (resources/ISO-3166.csv) is used to convert and standardize the names of the places to compose
# the map.
# The outputs files are three:
# "resources/countriesDF.xlsx", in the form:
# Country	Quantity	ID
# "resources/statesDF.xlsx", in the form:
# State		Quantity	ID
# "resources/errsDF.xlsx", in the form:
# ID	Error
# To use this section turn the variable $generate_db$ to True
generate_db = False
if generate_db:
...
m = choropleth_map(m, state_geo, 'choropleth_state', statesDF,
                   ['State', 'Quantity'], 'properties.name', 'Number of Tweets per state (Log10 scale)')
...
m = choropleth_map(m, state_geo, 'choropleth_country', countriesDF,
                   ['Country', 'Quantity'], 'properties.ISO_A3', 'Number of Tweets per country (Log10 scale)', fill_color="YlOrBr")
countries = pd.read_json("resources/countries.geojson")
...
def style_function(x): return {'fillColor': '#006600',
                               'color': '#000000',
                               'fillOpacity': 0.5,
                               'weight': 0.1}
